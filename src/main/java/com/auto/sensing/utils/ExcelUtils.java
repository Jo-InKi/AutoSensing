...
public class ExcelUtils implements ExcelUtilMethodFactory {
    ...
    * */
    @Override
    public void sensorExcelDownload(List<ExcelReportDTO> data, HttpServletResponse response) {
        // 엑셀파일(Workbook) 객체 생성
        ...
        renderSensorExcelBody(data, sheet, row, cell);

        // DownLoad
        // 엑셀 파일이 완성 되면 파일 다운로드를 위해 content-type과 Header를 설정해준다.
        // filename=파일이름.xlsx 가 파일의 이름이 된다.
        response.setContentType("ms-vnd/excel");
        response.setHeader("Content-Disposition", "attachment;filename=SensorReport.xlsx");
     ...
     * */
    @Override
    public void renderSensorExcelBody(List<ExcelReportDTO> data, Sheet sheet, Row row, Cell cell) {
        // 현재 행의 개수를 가지고 있는 변수 rowCount 선언(Header를 그리고 시작했으므로 1부터 시작)
        int rowCount = 1;

        // 조회해온 데이터 리스트(List<StudentDto>)의 크기만큼 반복문을 실행한다.
        for(ExcelReportDTO student : data) {
            ...
            cell.setCellValue(student.getSensorname());
            // 두 번째 cell(열)을 생성한다.
            ...
            cell.setCellValue(student.getValue());
            // 네 번째 cell(열)을 생성한다.
            cell = row.createCell(3);
            // 네 번째 cell(열)의 값을 셋팅한다.
            cell.setCellValue(student.getChange());
        }
    }
    ...
    * */
    @Override
    public List<ExcelReportDTO> readSensorExcel(MultipartFile file) {

        List<ExcelReportDTO> studentDtoList = new ArrayList<>();
        ...
        try {
            ...
            for(int i=1; i<sheet.getPhysicalNumberOfRows(); i++) {
                ...
                ExcelReportDTO studentDto = ExcelReportDTO.builder()
                        .sensorname(row.getCell(0).getStringCellValue())
                        .time(row.getCell(1).getStringCellValue())
                        .value((Double)row.getCell(2).getNumericCellValue())
                        .change((Double)row.getCell(3).getNumericCellValue())
                        .build();
